...
*/ 
$(function(){
	/*console.log(upimgAction);
	console.log(addAction);
	console.log(modifyStatusAction);
	console.log(seachByNameAction);
	console.log(modifyInitial);
	...
	$('#dataListTable').on('click','.modify',function(){
		...
		saleSellView.modifyBefore(this,dataId);
	});
	...
	$('.on-off').live('click',function(){
		...
		switch (statusAttr){
			case '1':
				...
				status = 0;
				saleSellView.openState(this,dataId,status);
			break;
			case '0':
	...
	$('.btn-info').live('click',function(){
		...
		if (statusAttr == -1) {
			...
			saleSellView.deleteState(this,dataId,statusAttr);
...
var saleSellView = {
	...
	addBuy:function(that,pic_big,title,brief,intro,good_id,type_id,deal_status,deal){
		...
		ajax_request(that,addAction,'post',{
			...
			if (msg.status && msg.status==1) {
				...
				GhostMsg(msg.info);
				modalView('hide' ,true);
				saleSellView.addData(0,data.id,pic_big,title,brief,intro,good_id,type_id,deal_status,0,0,data.admin_uname);
			}else{
				Loading('hide');
	...
	modifyEnd:function(that,dataId,pic_big,title,oldtitle,brief,intro,good_id,type_id,deal_status,recommend,like_count,admin_uname){
		...
		ajax_request(that,modifyAction,'post',{
			...
			if (msg.status && msg.status==1) {
				...
				modalView('hide' ,true);
				console.log(title+'a')
				saleSellView.addData(1,dataId,pic_big,title,brief,intro,good_id,type_id,deal_status,recommend,like_count,admin_uname);
			}else{
				Loading('hide');
	...
	submitData:function(dataId,good_id,type_id,deal_status,recommend,like_count,admin_uname){
		...
		if (modifyAtrr == 'modify') {
			// alert('修改1');
			saleSellView.modifyEnd(this,dataId,pic_big,title,oldtitle,brief,intro,good_id,type_id,deal_status,recommend,like_count,admin_uname);
			$('.btn_edit_submit').removeAttr('data-modify');
		}else{
			// alert('添加');
			saleSellView.addBuy(this,pic_big,title,brief,intro,good_id,type_id,0,1);
		};
...
var addHtml = function(){
	...
	if (!addhtml) {
	    ...
	    $('.btn_edit_submit').on('click',function(){
	    	...
	    	saleSellView.submitData(dataId,good_id,type_id,deal_status,recommend,like_count,admin_uname);
	    });
